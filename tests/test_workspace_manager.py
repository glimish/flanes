...
from vex.state import WorldStateManager
from vex.workspace import WorkspaceManager


@pytest.fixture
def env(tmp_path):
    """Provides (vex_dir, wm, wsm, store)."""
    vex_dir = tmp_path / ".vex"
    vex_dir.mkdir()
    db = vex_dir / "store.db"
    store = ContentStore(db)
    wsm = WorldStateManager(store, db)
    wm = WorkspaceManager(vex_dir, wsm)
    yield vex_dir, wm, wsm, store
    store.close()
...
class TestIsDirty:
    ...
    def test_returns_marker_when_materializing(self, env):
        ...
        (info.path / ".vex_materializing").write_text(json.dumps(marker))
    ...
    def test_returns_error_for_corrupt_marker(self, env):
        vex_dir, wm, wsm, store = env
        info = _create_workspace(wm, wsm, store)
        (info.path / ".vex_materializing").write_text("NOT JSON{{{")
        result = wm.is_dirty("ws1")
        assert result is not None
...
class TestIsLockStale:
    ...
    def test_stale_when_pid_dead_same_host(self, env):
        _, wm, _, _ = env
        from vex.workspace import _hostname
